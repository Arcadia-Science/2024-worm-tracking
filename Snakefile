...
FILEPATHS = find_input_files(
                input_dirpath = config["input_dirpath"],
                input_prefix_to_remove = INPUT_PREFIX
            )
OUTPUT_DIRPATH = Path(config["output_dirpath"])

rule convert_nd2_to_tiff:
    input: nd2 = INPUT_PREFIX + "/{filepath}.nd2" 
    output: tiff = OUTPUT_DIRPATH / "raw_tiff" / "{filepath}.tiff" 
    conda: "envs/dev.yml"
    shell:
        """
...
rule difference_of_gaussians_filter:
    ...
    output: dog = OUTPUT_DIRPATH / "dogfilter_tiff" / "{filepath}.tiff" 
    conda: "envs/dev.yml"
    shell:
        """
...
rule convert_tiff_to_mov:
    ...
    output: mov = OUTPUT_DIRPATH / "dogfilter_mov" / "{filepath}.mov"
    conda: "envs/dev.yml"
    shell:
        """
...
rule overlay_mov:
    ...
    output: png = OUTPUT_DIRPATH / "dogfilter_mov_overlay" / "{filepath}.png"
    conda: "envs/dev.yml"
    shell:
        """
